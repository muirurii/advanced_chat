...
const SignUp = ({setTab,setUser}:FormParams) => {
    ...
    const[passwordView,setPasswordView] = useState<{password:boolean,repeatPassword:boolean}>({password:false,repeatPassword:false})
    ...
    const handleSubmit = async (e:FormEvent<HTMLFormElement>):Promise<any>=>{
      e.preventDefault();
      if(!formData.username.length || !formData.password.length || !formData.repeatPassword.length){
    ...
    return (
      <form onSubmit={handleSubmit} className="w-full">
        ...
        <p className="text-red-400 text-sm h-8 py-2">{message.length ? message : null}</p>
          <div className="my-1">
              <input
          ...
          <div className="my-1 relative cursor-pointer">
              ...
              onChange={handleFormData}
              />
            <div onClick={()=> setPasswordView({...passwordView,password:!passwordView.password})} className="absolute top-1/2 -translate-y-1/2 right-2">
             {
          ...
          <div className="my-1 relative cursor-pointer">
              ...
              name="repeatPassword"
              />
            <div onClick={()=> setPasswordView({...passwordView,repeatPassword:!passwordView.repeatPassword})} className="absolute top-1/2 -translate-y-1/2 right-2">
             {
              passwordView.repeatPassword ? <RiEyeOffLine/> : <RiEyeLine/>
             }
            </div>
          </div>
          <button type="submit" className='bg-secondary mt-6 py-3 px-5 rounded flex items-center justify-center gap-x-1'>
            <span>SignUp</span>
            <BiLogIn/>  
          </button>
      </form>
