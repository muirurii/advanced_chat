...
const Profile = () => {
  ...
  const { state:{user:{isLogged,token}}, dispatch } = context;
  ...
  const addFriend = async(friendName:string) =>{
      try {
        const res = await customFetch("users/friends/add","POST",{friendName},token);
  ...
  return (
    <section className="min-h-screen min-w-screen gradient pt-[100px] flex flex-col items-center">
      ...
      {
      loading ?
       (<><ChatLoading/> <ChatLoading/> <ChatLoading/> <ChatLoading/> <ChatLoading/> <ChatLoading/><ChatLoading/> <ChatLoading/> <ChatLoading/></>) :
       users.length ?
       users.map((user)=> <Friend addFriend={addFriend} user={user} key={user._id} /> ) :
